...
**A new regression test passing on new code does not prove that the test would catch the old behavior.**

Change Proof is an open-source local CLI that records bounded evidence about whether explicitly selected changed tests distinguish an exact base implementation from an exact head implementation.
...
When State A and State B pass, State C observes the expected assertion failure, and the explicit boundary is valid, Change Proof records evidence that the selected changed tests distinguished the exact base implementation from the exact head implementation.

It does **not** prove overall implementation correctness, complete regression coverage, or the absence of other defects.

**Try the deterministic quickstart:** [`examples/quickstart/`](examples/quickstart/)

~~~bash
node examples/quickstart/run.mjs
~~~

Or install the current public beta:
...
`candidateSha256` is **not** the raw byte-for-byte SHA-256 of `candidate.json`. It is computed from the canonical serialization of the immutable `candidate.identity` payload; candidate metadata such as timestamps and durations is intentionally excluded. Therefore, `sha256sum candidate.json` is not expected to equal `candidateSha256`.
